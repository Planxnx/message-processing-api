...
import (
	...
	kafaPkg "github.com/Planxnx/message-processing-api/alarm-service/pkg/connection/kafka"
	"github.com/Planxnx/message-processing-api/alarm-service/pkg/connection/mongodb"
...
func InitializeConnection() *Connections {
	...
	if err != nil {
		panic(fmt.Errorf("InitializeConnection Error: failed on create kafka publisher connection: %v", err.Error()))
	}
	kafkaSubscriber, err := kafaPkg.NewSubscriber()
	if err != nil {
		panic(fmt.Errorf("InitializeConnection Error: failed on create kafka subsrciber connection: %v", err.Error()))
	}
